...
export default function GenerativeFeedbackTool({ showStatus, statusMessage, isLoading, setIsLoading, companyProfile, setCompanyProfile, pipelines, setPipelines }: GenerativeFeedbackToolProps) {
    ...
    const statusClasses = useMemo(() => {
        switch (applicationStatus) {
            case 'HIRED': return 'bg-green-100 border-green-400 text-green-800';
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="p-6 bg-white rounded-xl shadow-lg border border-gray-200 h-full">
                <h2 className="text-2xl font-semibold text-gray-800 flex items-center mb-5 border-b pb-3">
            ...
            <div className="p-6 bg-white rounded-xl shadow-lg border border-gray-200 h-full flex flex-col">
                ...
                <div className="flex-grow relative">{isLoading && (<div className="absolute inset-0 bg-white bg-opacity-80 flex items-center justify-center rounded-lg z-10"><Loader2 className="w-8 h-8 text-indigo-600 animate-spin" /><span className="ml-3 text-lg text-indigo-600">Generating...</span></div>)}<textarea value={feedbackMessage} onChange={(e) => setFeedbackMessage(e.target.value)} rows={15} className="w-full h-full min-h-[300px] rounded-md border-gray-300 bg-gray-50 shadow-inner p-4 text-gray-700 resize-none font-sans text-sm focus:bg-white" placeholder="Your personalized, constructive feedback message will appear here after generation. This field is editable."/></div>
                <div className="mt-4 flex flex-col sm:flex-row gap-3">
                    <button onClick={handleCopyFeedback} disabled={!feedbackMessage || isLoading} className="flex-1 flex items-center justify-center px-4 py-3 border border-transparent text-base font-medium rounded-lg text-indigo-600 bg-indigo-100 hover:bg-indigo-200 transition duration-150 shadow-md disabled:opacity-50 disabled:cursor-not-allowed"><Clipboard className="w-5 h-5 mr-2" />Copy Edited Message</button>
                     <div className="flex-1 flex flex-col gap-2">
                         <select value={pipelineToSaveTo} onChange={(e) => setPipelineToSaveTo(e.target.value)} className="w-full p-3 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" disabled={pipelineNames.length === 0}>
