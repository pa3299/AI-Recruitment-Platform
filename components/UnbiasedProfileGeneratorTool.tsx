...
The final output must be in clean, readable Markdown. Use headings for each section (e.g., \`## Work Experience\`, \`## Skills\`, \`## Languages\`). Do not add any commentary or explanation outside of the requested structured output.`;
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-semibold text-gray-800 flex items-center mb-4"><ShieldCheck className="w-6 h-6 mr-2 text-indigo-600" />Unbiased Profile Generator ✨</h2>
            <div className="p-6 bg-white rounded-xl shadow-lg border border-gray-200">
                <p className="text-sm text-gray-500 p-2 bg-indigo-50 rounded-lg border border-indigo-200 mb-4">Provide the candidate's name, then upload their documents or paste their CV text to generate a bias-free profile.</p>
                <label className="block mb-4"><span className="text-gray-700 font-medium">Candidate Name (for Pipeline)</span><input type="text" value={candidateName} onChange={(e) => setCandidateName(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-3" placeholder="e.g., John Doe"/></label>
                ...
                <label className="block"><span className="text-gray-700 font-medium">Or Paste Raw CV/Cover Letter Text</span><textarea value={rawText} onChange={(e) => setRawText(e.target.value)} rows={8} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-4 font-mono text-sm" placeholder="Paste full CV text here if you don't have a file." /></label>
                <button onClick={handleGenerate} disabled={isGenerateDisabled} className="mt-6 w-full flex items-center justify-center px-4 py-3 border border-transparent text-base font-medium rounded-lg text-white bg-teal-600 hover:bg-teal-700 transition duration-150 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">{isLoading ? (<Loader2 className="w-5 h-5 mr-2 animate-spin" />) : (<ShieldCheck className="w-5 h-5 mr-2" />)}Generate Unbiased Profile & Fit Summary</button>
            </div>
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 space-y-6">
                 {(anonymizedResult && fitSummaryResult) && (
                     <div className="border-b pb-4 flex flex-col sm:flex-row gap-4 items-center justify-end">
                        <select value={pipelineToSaveTo} onChange={(e) => setPipelineToSaveTo(e.target.value)} className="w-full sm:w-auto p-2 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" disabled={pipelineNames.length === 0}>
