    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <h2 className="text-2xl font-semibold text-gray-800 flex items-center"><Users className="w-6 h-6 mr-2 text-indigo-600" />Candidate Pipeline</h2>
                {pipelineNames.length > 0 && (<select value={activePipeline || ''} onChange={(e) => setActivePipeline(e.target.value)} className="p-2 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{pipelineNames.map(name => <option key={name} value={name}>{name}</option>)}</select>)}
            </div>
            ...
            {!activePipeline ? (
                <div className="text-center p-10 bg-white rounded-xl shadow-lg border border-gray-200"><Users className="w-12 h-12 mx-auto text-gray-300" /><h3 className="mt-4 text-lg font-medium text-gray-800">No pipelines created yet.</h3><p className="mt-1 text-sm text-gray-500">Create your first pipeline above to start organizing candidates.</p></div>
            ) : currentProfiles.length === 0 ? (
                <div className="text-center p-10 bg-white rounded-xl shadow-lg border border-gray-200"><Users className="w-12 h-12 mx-auto text-gray-300" /><h3 className="mt-4 text-lg font-medium text-gray-800">The "{activePipeline}" pipeline is empty.</h3><p className="mt-1 text-sm text-gray-500">Go to another tool to save profiles or feedback to this pipeline.</p></div>
            ) : (
                <div className="space-y-4">
                    {currentProfiles.map((profile, index) => {
                        ...
                        return (
                             <div key={profile.id} draggable={!isEditing} onDragStart={(e) => handleDragStart(e, index)} onDragEnter={(e) => handleDragEnter(e, index)} onDragEnd={handleDragEnd} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} className="p-6 bg-white rounded-xl shadow-lg border border-gray-200 transition-all duration-200 hover:border-indigo-400 hover:shadow-xl group">
